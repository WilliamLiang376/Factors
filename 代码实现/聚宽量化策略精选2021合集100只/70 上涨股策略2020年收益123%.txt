...
# 原文网址：https://www.joinquant.com/post/30863
# 标题：上涨股策略2020年收益123%
# 作者：scottchenrui
...
def get_up_security(context):
    ...
    for s in dfs300:#dfs.index:
        df=get_price(s, end_date=context.previous_date, frequency='daily', fields=["close"], skip_paused=False, fq='pre',count=g.check_stocks_refresh_rate)
...
def get_up_stocks(context):
    ...
    for s in dfs300:#dfs.index:
        ...
        #转换成numpy的ndarray数据格式，n行1列,LinearRegression需要列格式数据，如下：
        X_train = np.array(X).reshape((len(X), 1))
        Y_train = np.array(x).reshape((len(x), 1))
        #新建一个线性回归模型，并把数据放进去对模型进行训练
        lineModel = LinearRegression()
        lineModel.fit(X_train, Y_train)
        ...
        score=lineModel.score(X_train, Y_train)
        #print("y=%.4f*x+%.4f" % (a1,b))
        if a1>1.5 and lineModel.score(X_train, Y_train)>0.7:
            temp_index.append(s)
    return sorted(list(set(temp_index)))
# 行业过滤
